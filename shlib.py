...
def get_house_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"ershoufang/d%drs%s/" % (page, urllib.request.quote(communityname.encode('utf8')))
...
def get_sell_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"chengjiao/d%drs%s/" % (page, urllib.request.quote(communityname.encode('utf8')))
            source_code = misc.get_source_code(url_page)
            soup = BeautifulSoup(source_code, 'lxml')
        i = 0
...
def get_rent_percommunity(communityname):
    ...
    for page in range(total_pages):
        if page > 0:
            url_page = BASE_URL + u"rent/d%drs%s/" % (page, urllib.request.quote(communityname.encode('utf8')))
            source_code = misc.get_source_code(url_page)
            soup = BeautifulSoup(source_code, 'lxml')
        i = 0
